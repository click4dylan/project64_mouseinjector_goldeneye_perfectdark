...
static void queue_from_rawinput(const RAWINPUT *raw)
{
    ...
    for (i = 0; i < available_devices; i++)  /* find the device for event. */
    {
        if (mice[i].handle == header->hDevice)
            break;
    } /* for */

    if (i == available_devices)
        return;  /* not found?! */
	...
	event.iskeyboard = mice[i].iskeyboard;
	if(raw->header.dwType == RIM_TYPEKEYBOARD)
	{
		event.type = MANYMOUSE_EVENT_KEYBOARD;
		event.item = keyboard->VKey;
		if(keyboard->Message == WM_KEYDOWN || keyboard->Message == WM_SYSKEYDOWN) // pressed down
			event.value = 1;
		else if(keyboard->Message == WM_KEYUP || keyboard->Message == WM_SYSKEYUP) // pressed up
			event.value = 0;
...
{
    const char rdp_ident_kb[] = "ROOT\\RDP_KBD\\";
    const char rdp_ident_mi[] = "ROOT\\RDP_MOU\\";
    MouseStruct *mouse = &mice[available_devices];
    char *buf = NULL;
    char *ptr = NULL;
    UINT ct = 0;

    if (dev->dwType != RIM_TYPEMOUSE && dev->dwType != RIM_TYPEKEYBOARD)
        return;  /* if not keyboard or mice (some other fruity thing). */
    ...
    *ptr = '\0';
    ...
    if (ct >= sizeof (rdp_ident_kb) - 1)
    {
        int i;
        for (i = 0; i < sizeof (rdp_ident_kb) - 1; i++)
        {
            if (buf[i] != rdp_ident_kb[i])
                break;
        } /* for */

        if (i == sizeof (rdp_ident_kb) - 1)
            return;  /* this is an RDP thing. Skip this device. */
    } /* if */
    if (ct >= sizeof (rdp_ident_mi) - 1)
    {
        int i;
        for (i = 0; i < sizeof (rdp_ident_mi) - 1; i++)
        {
            if (buf[i] != rdp_ident_mi[i])
                break;
        } /* for */

        if (i == sizeof (rdp_ident_mi) - 1)
            return;  /* this is an RDP thing. Skip this device. */
    } /* if */
	...
	mouse->iskeyboard = dev->dwType == RIM_TYPEKEYBOARD ? 1 : 0;
    available_devices++;
} /* init_mouse */
...
static void windows_wminput_quit(void)
{
    ...
    rid[1].hwndTarget = raw_hwnd;
    pRegisterRawInputDevices(rid, 2, sizeof (RAWINPUTDEVICE));
    cleanup_window();
    available_devices = 0;
    pDeleteCriticalSection(&mutex);
...
static const char *windows_wminput_name(unsigned int index, unsigned int iskeyboard)
{
    return (index < available_devices && mice[index].iskeyboard == iskeyboard) ? mice[index].name : NULL;
...
static int check_for_disconnects(ManyMouseEvent *ev)
{
    ...
    if (++i >= available_devices)  /* check first in case of redetect */
        i = 0;
    ...
    if (mouse->handle != NULL)  /* not NULL == still plugged in. */
    {
        ...
        if (rc == (UINT) -1)  /* failed...probably unplugged... */
        {
            ...
            ev->device = i;
            ev->iskeyboard = mouse->iskeyboard;
            return 1;
        } /* if */
    } /* if */
